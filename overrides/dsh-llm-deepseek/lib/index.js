...
function apply(ctx, config) {
	...
	ctx.provide(`${PROVIDER}.balance`, {
		getBalance: async (signal) => {
			...
			const parseAmount = (value) => typeof value === "number" ? value : typeof value === "string" && value !== "" && Number.isFinite(Number(value)) ? Number(value) : void 0;
			const granted = parseAmount(info?.granted_balance);
			const toppedUp = parseAmount(info?.topped_up_balance);
			const totalBalance = parseAmount(info?.total_balance) ?? (granted !== void 0 && toppedUp !== void 0 ? granted + toppedUp : void 0);
			if (totalBalance === void 0) throw new LlmError(`DeepSeek balance response missing total_balance: ${JSON.stringify(data)}`, "INVALID_RESPONSE");
			return {
