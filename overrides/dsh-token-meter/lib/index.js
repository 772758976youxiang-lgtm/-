...
*/
function isDeepSeekPeakAt(timeMs) {
	const beijing = new Date(timeMs + 8 * 3600e3);
...
function deepseekUsageCost(buckets, timeMs, model) {
	const base = DEEPSEEK_RATES[model] ?? DEEPSEEK_DEFAULT_RATE;
	const factor = isDeepSeekPeakAt(timeMs) ? 1 : 0.5;
	return (buckets.cacheReadTokens / 1e6 * base.inputCacheHit + (buckets.uncachedInputTokens + buckets.cacheWriteTokens) / 1e6 * base.inputCacheMiss + buckets.outputTokens / 1e6 * base.output) * factor;
}
...
const tokenUsageViewSchema = projectionSchema.extend({
	cost: z$1.number().nonnegative().optional()
}).strict().transform(({ cost, ...totals }) => cost === void 0 ? totals : { ...totals, cost });
/**
...
const tokenUsageProjectionDefinition = {
	...
	apply: (state, event) => {
		...
		if (previous !== void 0 && bucketsEqual(previous.buckets, buckets)) return state;
		const priced = state.route !== null && state.route.provider === "deepseek-official" ? deepseekUsageCost(buckets, event.time, state.route.model) : 0;
		return {
