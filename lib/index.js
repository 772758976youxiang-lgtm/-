...
function startWechatProgressMonitor(exec, baseUrl, token, channelId, monitors) {
  ...
  const followup = (text) => {
    ...
    exec.agent.followup({ id, role: "user", content: [{ type: "text", text }], source: { kind: "plugin", plugin: "@deepseek-ai/dsh-channel-im", form: "notice", summary: "微信机器人搭建进度" } });
  };
  const check = async () => {
    try {
      ...
      followup(`这是微信机器人每 1 分钟自动检查的进度：${onboardingSummary(bot)}。请直接向用户简短报告当前步骤。`);
      if (["connected", "error", "stopped"].includes(bot.phase)) { clearInterval(monitors.get(channelId)); monitors.delete(channelId); }
    } catch (error) {
      followup(`微信机器人定时检查遇到问题：${error?.message || error}。请直接向用户简短报告。`);
    }
  };
  ...
  ctx.tools.register({
    ...
    parameters: {
      ...
      properties: {
        action: {
          type: "string",
          enum: ["status", "setup_wechat", "stop_wechat", "send_wechat_message", "configure_dingtalk_robot", "begin_dingtalk_person_login", "remove_channel"],
