...
export default function ConnectionSection({ t, readStatus, readWechat, readRules, saveRules, readContacts }) {
  ...
  React.useEffect(() => {
    if (!rulesOpen || !activeChannelId) return;
    Promise.all([readRules(activeChannelId), readContacts(activeChannelId)]).then(([saved, found]) => {
      if (saved?.policy) setRules((old) => ({ ...old, ...saved.policy }));
      setContacts(found?.items || []);
    }).catch((e) => setError(String(e)));
  }, [rulesOpen, activeChannelId]);
  const persistRules = async (next) => { if (!activeChannelId) return; setRules(next); const saved = await saveRules(activeChannelId, next); if (saved?.policy) setRules((old) => ({ ...old, ...saved.policy })); };
  ...
  const connectedBots = (wechat.bots || []).filter((bot) => bot.phase === "connected");
  ...
  return (
    <div>
      ...
      {connectedBots.map((bot) => { const opened = rulesOpen && activeChannelId === bot.channelId; return <div key={bot.channelId} style={rulesCardStyle}>
        <button type="button" onClick={() => { setActiveChannelId(bot.channelId); setRulesOpen(!opened); setError(""); }} style={{ width: "100%", minHeight: "56px", border: 0, background: "transparent", padding: "0 16px", display: "flex", alignItems: "center", justifyContent: "space-between", cursor: "pointer", color: "var(--dsw-alias-label-primary)", fontWeight: 600, fontSize: "14px" }}><span style={{ display: "flex", alignItems: "center", gap: "8px" }}>{bot.account?.nickname || bot.name || t("wechat.name")}{dot()}<span style={{ color: "var(--dsw-alias-state-success-primary)", fontSize: "12px", fontWeight: 500 }}>{t("wechat.connected")}</span></span><span style={{ color: "var(--dsw-alias-label-secondary)", fontWeight: 400, fontSize: "13px" }}>{opened ? t("collapse") : t("configure")}</span></button>
        {opened && <div style={{ padding: "4px 16px 16px", borderTop: "1px solid var(--dsw-alias-border-l2)" }}>
          {(error || bot.error) && <div role="alert" style={{ fontSize: "12px", color: "var(--dsw-alias-state-error-primary, #dc2626)", margin: "12px 0 0" }}>{error || bot.error}</div>}
...
/* locale（settings 命名空间，zh + en）：
   zh: external.nav="连接" external.title="连接" external.intro="连接后可在这里配置通道。"
   en: external.nav="Connections" external.title="Connections" external.intro="Connected channels can be configured here."
*/
