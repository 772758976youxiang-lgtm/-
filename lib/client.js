...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ExternalSection({ t, readStatus, readWechat, toggleWechat, readRules, saveRules, readContacts }) {
      ...
      var rulesState = react.useState({ direct_message: "allow", group_message: "allow", direct_whitelist: [], direct_blacklist: [], group_whitelist: [], group_blacklist: [], group_reply_only_when_mentioned_groups: [] });
      ...
      return jsx.jsxs("div", { children: [
        ...
        jsx.jsxs("div", { style: { border: "1px solid var(--dsw-alias-border-l2)", borderRadius: "14px", marginBottom: "12px", overflow: "hidden" }, children: [
          jsx.jsxs("button", { type: "button", onClick: function () { setRulesOpen(!rulesOpen); }, style: { width: "100%", border: 0, background: "var(--dsw-alias-bg-layer-1)", padding: "12px 16px", display: "flex", justifyContent: "space-between", cursor: "pointer", color: "var(--dsw-alias-label-primary)", fontWeight: "600" }, children: ["群聊与联系人规则", jsx.jsx("span", { children: rulesOpen ? "收起" : "展开" })] }),
          rulesOpen ? jsx.jsxs("div", { style: { padding: "16px", background: "var(--dsw-alias-bg-layer-1)", borderTop: "1px solid var(--dsw-alias-border-l2)" }, children: [
            jsx.jsx("div", { style: { fontSize: "13px", color: "var(--dsw-alias-label-tertiary)", marginBottom: "12px" }, children: "黑名单优先于白名单。群聊上下文由通道后台保存最近 200 条，不在此设置面板展示。" }),
            ...
            ["group", "direct"].map(function(kind){ var rows=contacts.filter(function(c){return c.type===kind;}); return jsx.jsxs("section", { style: { marginTop: "14px" }, children: [jsx.jsx("div", { style: { fontWeight: "600", marginBottom: "6px" }, children: kind === "group" ? "群聊列表" : "联系人列表" }), rows.map(function(c){ var w=(rules[kind+"_whitelist"]||[]).indexOf(c.id)>=0, b=(rules[kind+"_blacklist"]||[]).indexOf(c.id)>=0, mention=(rules.group_reply_only_when_mentioned_groups||[]).indexOf(c.id)>=0; return jsx.jsxs("div", { key:c.id, style:{display:"flex",gap:"8px",alignItems:"center",padding:"8px 0",fontSize:"13px",borderTop:"1px solid var(--dsw-alias-border-l2)",flexWrap:"wrap"}, children:[jsx.jsx("span", {style:{flex:"1",minWidth:"180px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}, children:(c.remark||c.nickname||c.name||c.id)+" · "+c.id}), kind === "group" ? jsx.jsxs("label",{style:{fontSize:"12px"},children:[jsx.jsx("input",{type:"checkbox",checked:mention,onChange:function(e){setGroupMention(c.id,e.target.checked);}})," 仅 @AI 回复"]}):null, jsx.jsx("button",{type:"button",onClick:function(){setMember(kind,c.id,"white");},children:w?"白名单✓":"白名单"}), jsx.jsx("button",{type:"button",onClick:function(){setMember(kind,c.id,"black");},children:b?"黑名单✓":"黑名单"}), jsx.jsx("button",{type:"button",onClick:function(){setMember(kind,c.id,"clear");},children:"清除"})]}); })] }, kind); })
          ] }) : null
        ] }),
        shown.length === 0
          ? null
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject("settings.section", function () {
        return ctx.slots.register({ name: "settings.section", id: "external-connections", order: 20, label: function () { return "连接"; }, inject: function () { return { t: t, readStatus: readStatus, readWechat: readWechat, toggleWechat: toggleWechat, readRules: readRules, saveRules: saveRules, readContacts: readContacts }; } }, ExternalSection);
      });
    }
