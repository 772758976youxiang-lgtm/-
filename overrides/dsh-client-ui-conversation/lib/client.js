window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const BALANCE_POLL_MS = 3e4;
		/** Peak-hour CNY price per 1M tokens, keyed by provider then model. Off-peak = half (only DeepSeek has peak/off-peak today). */
		const PROVIDER_MODEL_PRICES = {
			"deepseek-official": {
				"deepseek-v4-flash": { inputCacheHit: 0.1, inputCacheMiss: 3.0, output: 9.0 },
				"deepseek-v4-flash-vision-exp": { inputCacheHit: 0.1, inputCacheMiss: 3.0, output: 9.0 },
				"deepseek-v4-pro": { inputCacheHit: 0.3, inputCacheMiss: 9.0, output: 27.0 }
			}
		};
		/** Fall back to this when the active provider/model has no pricing entry. */
		const DEFAULT_PRICE = { inputCacheHit: 0.1, inputCacheMiss: 3.0, output: 9.0 };
		/** Whether `now` is a DeepSeek peak hour in Beijing time (UTC+8): 09:00–12:00, 14:00–18:00. Since 2026-08-23, weekends (Sat/Sun) are all-day off-peak: Sat/Sun 全天的峰谷不再区分，统一按谷价计费。 */
		...
		/** Current per-1M rates for the active provider/model, applying the DeepSeek peak/off-peak factor at `now`. */
		function tokenMoneyRates(now, provider, model) {
		...
		/** Session billed-token cost, split by cache-hit / cache-miss / output pricing at `now` for the active model. */
		function sessionCost(usage, now, provider, model) {
			const rates = tokenMoneyRates(now, provider, model);
			const hit = (usage.cacheReadTokens ?? 0) / 1e6 * rates.inputCacheHit;
			const miss = ((usage.uncachedInputTokens ?? 0) + (usage.cacheWriteTokens ?? 0)) / 1e6 * rates.inputCacheMiss;
			const out = (usage.outputTokens ?? 0) / 1e6 * rates.output;
			return hit + miss + out;
		}
		...
		function moneyText(currency, value) {
			return value === void 0 ? "—" : `${currencySymbol(currency)}${value.toFixed(2)}`;
		}
		...
		function codeLang(path) {
			...
			const map = {
				...
				xml: "xml", svg: "xml", py: "python", sh: "bash", bash: "bash",
				zsh: "bash", yaml: "yaml", yml: "yaml", toml: "toml", sql: "sql",
				go: "go", rs: "rust", java: "java", c: "c", h: "c", cpp: "cpp",
				cc: "cpp", hpp: "cpp", rb: "ruby", php: "php", cs: "csharp", kt: "kotlin", swift: "swift"
			};
			return map[ext];
		}
		/** Compact human byte size. */
		function formatBytes(bytes) {
			if (!Number.isFinite(bytes) || bytes === void 0) return "";
			if (bytes < 1024) return `${bytes} B`;
			if (bytes < 1024 * 1024) return `${(bytes / 1024).toFixed(1)} KB`;
			return `${(bytes / (1024 * 1024)).toFixed(2)} MB`;
		}
		/** File viewer: above this char count, skip syntax highlighting (it's the main jank source). */
		const FILE_VIEWER_HIGHLIGHT_LIMIT = 4e4;
		/** File viewer: chars rendered per slice before "load more". */
		const FILE_VIEWER_SLICE = 6e4;
		/**
			...
			return (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Tooltip, {
				...
				children: (0, react_jsx_runtime.jsx)("div", {
					...
					(0, react_jsx_runtime.jsx)("span", {
						style: peak ? { color: "rgb(229,72,77)" } : { color: "rgb(48,164,108)" },
						children: peakLabel
					}),
					...
					...groups.map((group, i) => (0, react_jsx_runtime.jsxs)(react.Fragment, { children: [i > 0 && (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)("span", {
						className: StatsLine_module_css_default.sep,
						"aria-hidden": true,
						children: "|"
		...
		*/
		...
		function ContextMeter({ useProjection, t, loadBalance }) {
			...
			(0, react.useEffect)(() => {
				if (typeof loadBalance !== "function") return;
				let alive = true;
				const refresh = () => {
					try {
						...
						(0, react_jsx_runtime.jsx)("dl", {
							...
							children: [
								(0, react_jsx_runtime.jsxs)("div", {
									className: ContextMeter_module_css_default.row,
									children: [
										(0, react_jsx_runtime.jsx)("dt", { children: t("context.balance") }),
										(0, react_jsx_runtime.jsx)("dd", { children: balance !== null && balance.available === true && balance.totalBalance !== void 0 ? moneyText(balance.currency, balance.totalBalance) : balance !== null ? balance.message ?? "—" : "…" })
									]
								}),
								usage !== void 0 && (0, react_jsx_runtime.jsxs)("div", {
									className: ContextMeter_module_css_default.row,
									children: [
										(0, react_jsx_runtime.jsx)("dt", { children: t("context.sessionUsed") }),
										(0, react_jsx_runtime.jsx)("dd", { children: `${usage.cost !== void 0 ? moneyText("CNY", usage.cost) : moneyText(balance?.currency, sessionCost(usage, new Date(), balance?.provider, balance?.model))} · ${formatTokens(billedInputTokens(usage) + usage.outputTokens)} tok` })
									]
								})
							]
						})
		...
		*/
		...
		/** Chinese display labels for the built-in permission presets. */
		...
		function optionLabel(option) {
			const label = PERMISSION_OPTION_LABELS[option.value];
		...
		*/
		...
		function InputBar({ useSession, useInput, inputActions, keyboard, addImages, removeImage, draftImages, resolveSubmitMode, toggleCommandMenu, stop, command, t, renderSlot, useNotices, useLexicon, useMenuLauncher, useProjection, loadBalance, sessionId, variant, disabled: inert = false, blocked, workspacePickerOpen = false, onRequestWorkspace, placeholder, accessory, overlay, leftItems, rightItems, footer }) {
		...
		* has not exposed a visible box yet. */
		...
		function TurnStatus({ startTime, t }) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				role: "status",
				"aria-live": "polite",
				children: [t("stats.burningTokens"), showClock && (0, react_jsx_runtime.jsx)("span", {
		...
		*/
		...
		const zh = {
			...
			"context.used": "上下文已用",
			"context.balance": "账户余额",
			"context.sessionUsed": "本会话已用",
			...
			"stats.burningTokens": "你的 Harness 正在疯狂燃烧 token…",
			"stats.peak": "峰",
			"stats.valley": "谷",
			...
			"details.running": "运行中…",
			"details.loading": "加载中…",
			"details.truncated": "内容过长已截断（仅显示前 1MB）",
			"details.wrap": "换行显示",
			"details.wrapLabel": "换行",
			"details.loadMore": "加载更多（剩余 {remaining} 字符）",
			"todo.title": "任务",
		...
		const en = {
			...
			"context.sessionUsed": "Used this session",
			...
			"stats.burningTokens": "Your Harness is burning through tokens…",
			"stats.peak": "Peak",
			"stats.valley": "Off-peak",
			...
			"details.truncated": "Content truncated (showing the first 1 MB only)",
			"details.wrap": "Wrap text",
			"details.wrapLabel": "Wrap",
			"details.loadMore": "Load more ({remaining} characters remaining)",
			"todo.title": "To-dos",
		...
		*/
		...
		/** Virtualized code viewer: render only visible lines + a line-number gutter, with lightweight per-line highlighting. */
		const FILE_LINE_HEIGHT = 20;
		const FILE_OVERSCAN = 10;
		const FILE_MONO = "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace";
		const CODE_TOKEN_RE = /(\/\/.*$|#.*$)|("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*'|`(?:[^`\\]|\\.)*`)|\b(0x[0-9a-fA-F]+|\d+\.?\d*(?:[eE][+-]?\d+)?)\b|\b(const|let|var|function|return|if|else|for|while|class|import|from|export|new|async|await|try|catch|throw|type|interface|extends|implements|public|private|readonly|static|this|null|undefined|true|false|void|of|in|typeof|instanceof|switch|case|break|continue|default|do|yield|enum|delete|require|module|def|package|using|func|struct|fn)\b/g;
		const CODE_TOKEN_COLOR = { comment: "var(--dsw-alias-label-tertiary)", string: "rgb(63,185,80)", number: "rgb(88,166,255)", keyword: "rgb(255,123,114)" };
		/** Lightweight per-line syntax highlighter (approximate: comments/strings/numbers/keywords). */
		function highlightLine(line) {
			...
			while ((m = CODE_TOKEN_RE.exec(line)) !== null) {
				...
				const seg = m[0];
				const cls = m[1] !== void 0 ? "comment" : m[2] !== void 0 ? "string" : m[3] !== void 0 ? "number" : "keyword";
				out.push((0, react_jsx_runtime.jsx)("span", { style: { color: CODE_TOKEN_COLOR[cls] }, children: seg }, out.length));
				last = m.index + seg.length;
			}
			if (last < line.length) out.push(line.slice(last));
			return out;
		}
		function VirtualCode({ content }) {
			...
			const start = Math.max(0, Math.floor(top / FILE_LINE_HEIGHT) - FILE_OVERSCAN);
			const end = Math.min(lines.length, Math.ceil((top + viewport) / FILE_LINE_HEIGHT) + FILE_OVERSCAN);
			const rows = [];
			...
			return (0, react_jsx_runtime.jsx)("div", {
				...
				style: { flex: "1 1 auto", minHeight: 0, overflow: "auto", fontFamily: FILE_MONO, fontSize: "13px", lineHeight: `${FILE_LINE_HEIGHT}px`, position: "relative", tabSize: 2 },
				children: (0, react_jsx_runtime.jsx)("div", {
					style: { height: `${lines.length * FILE_LINE_HEIGHT}px`, position: "relative" },
					children: (0, react_jsx_runtime.jsxs)("div", {
						style: { transform: `translateY(${start * FILE_LINE_HEIGHT}px)` },
						children: rows.map((i) => (0, react_jsx_runtime.jsxs)("div", {
							key: i,
							style: { display: "flex", height: `${FILE_LINE_HEIGHT}px`, whiteSpace: "pre" },
							children: [
								(0, react_jsx_runtime.jsx)("span", {
									style: { minWidth: "46px", flex: "none", textAlign: "right", paddingRight: "12px", color: "var(--dsw-alias-label-tertiary)", userSelect: "none", fontVariantNumeric: "tabular-nums" },
									children: String(i + 1)
								}),
								(0, react_jsx_runtime.jsx)("span", {
									style: { flex: "1 1 auto", whiteSpace: "pre", color: "var(--dsw-alias-label-primary)" },
									children: highlightLine(lines[i])
								})
							]
						}, i))
					})
		...
		function DetailsPanel({ useSession, useSessions, sessionId, useStore, renderSlot, closeDetails, openPath, t }) {
				...
				return (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						(0, react_jsx_runtime.jsxs)("div", {
							...
							children: file.error !== void 0 ? (0, react_jsx_runtime.jsx)("div", {
								className: DetailsPanel_module_css_default.empty,
								children: file.error
							}) : file.loading === true || file.content === void 0 ? (0, react_jsx_runtime.jsx)("div", {
								className: DetailsPanel_module_css_default.empty,
								...
								children: formatBytes(file.byteLength)
							}), file.truncated === true && (0, react_jsx_runtime.jsx)("div", {
								className: DetailsPanel_module_css_default.sectionLabel,
								children: t("details.truncated")
							}), (0, react_jsx_runtime.jsx)(VirtualCode, {
			...
			slots.register({
				...
				inject: (sessionId) => {
					...
					return {
						...
						loadBalance: async () => {
							const response = await workspaces.api.host.balance({});
							if (!response.result.ok) throw new Error(response.result.error?.message ?? "余额查询失败");
				...
				inject: (sessionId, actions) => {
					...
					return {
						...
						openFile: (path) => {
							...
							const abs = (0, _deepseek_ai_dsh_client_runtime_client.resolveWorkspacePath)(cwd, path);
							if (path === ".") return workspaces.openPath(abs);
							const title = path.split(/[/\\]/).filter(Boolean).pop() ?? path;
							actions.select({ file: { path: abs, title, loading: true } });
							layout.openDetails();
							return workspaces.api.host.readFile({ path: abs }).then((res) => {
								if (!res.result.ok) throw new Error(res.result.error?.message ?? "读取失败");
								actions.select({ file: { path: abs, title, ...res.result.value, loading: void 0 } });
							}).catch((error) => {
								actions.select({ file: { path: abs, title, error: error instanceof Error ? error.message : String(error) } });
							});
						},
						loadOlder: () => {
							scoped.loadOlder();
