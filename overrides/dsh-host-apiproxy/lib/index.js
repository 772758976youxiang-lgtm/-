	...
	return {
		...
		host: {
			...
			async balance(request, signal) {
				...
				const balance = provider === void 0 ? void 0 : ctx.get(`${provider}.balance`);
				if (balance === void 0) return ok(request, { available: false, provider, message: "当前平台不提供余额查询" });
				try {
					const value = await balance.getBalance(signal);
					return ok(request, { available: value.available, provider, model: selection.model, currency: value.currency, totalBalance: value.totalBalance });
				} catch (error) {
					if (signal.aborted) return err(request, { code: "cancelled", message: "余额查询已取消", details: {} });
					return err(request, { code: "internal", message: `余额查询失败: ${error instanceof Error ? error.message : String(error)}`, details: {} });
				}
			},
			async readFile(request, signal) {
				...
				try {
					...
					if (!info.isFile()) return ok(request, { path, content: "", truncated: false, byteLength: 0, error: "not-a-file" });
					const buffer = await readFile(path);
					const byteLength = buffer.length;
					const truncated = byteLength > MAX;
					const slice = truncated ? buffer.subarray(0, MAX) : buffer;
					const content = slice.toString("utf8");
					if (content.includes("\u0000")) return ok(request, { path, content: "", truncated: false, byteLength, error: "binary" });
					return ok(request, { path, content, truncated, byteLength });
				} catch (error) {
					if (signal.aborted) return err(request, { code: "cancelled", message: "读取已取消", details: {} });
					return err(request, { code: "internal", message: `读取失败: ${error instanceof Error ? error.message : String(error)}`, details: {} });
				}
			}
		},
		goals: {
...
const hostBalanceValueSchema = z$1.object({
	...
	totalBalance: z$1.number().optional(),
	message: z$1.string().optional()
});
/** host.readFile request payload: an absolute path the Host should read. */
const hostReadFileRequestSchema = z$1.object({ path: z$1.string().min(1) });
/** host.readFile response value: the file's text content, or an error note. */
const hostReadFileValueSchema = z$1.object({
	path: z$1.string(),
	content: z$1.string(),
	truncated: z$1.boolean(),
	byteLength: z$1.number().int().nonnegative(),
...
const UNARY_ROUTES = {
	...
	"host.readFile": {
		...
		invoke: (api, r, signal) => api.host.readFile(r, signal)
	},
	"workspace.list": {
