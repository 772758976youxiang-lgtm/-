...
| 普通图片密钥探测 | 通过，后台线程读取微信进程/缓存，不调用 UIA、不阻塞消息轮询 |
| 管理 API 八个端点 | 通过 |
...
- 通道长期设定回归：专属预设能真实挂载 `channel_self_profile_update`；只有后台选择的唯一联系人私聊 Session 可写，其他 Session 在工具层被拒绝。写入前判断长期价值，落盘内容同时包含整体摘要和经合并、去重、纠错、移除过时内容后的完整设定，并在下一模型步骤重新读取。
