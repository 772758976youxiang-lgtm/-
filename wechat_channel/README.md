...
安装为 DSH 插件后，也可直接进入「设置 → 连接」打开“微信个人号”开关。插件会动态定位 `Weixin.exe` 并严格校验 `4.1.10.27`；版本不符时开关保持关闭，并提示兼容性与封号风险。用户确认后，插件才从固定 HTTPS 链接临时下载并校验安装包，通过一次管理员授权安装目标版本、关闭 `WeixinUpdate.exe` 自动更新，复检成功后自动打开通道。安装包不内置到插件，临时文件会清理。

版本正确时，插件只关闭现有 `Weixin.exe` 进程树，再启动新的可见登录窗口，等待扫码或手机确认，并在数据库可读后把状态切换为“已连接”。它不会结束旧版 `WeChat.exe` 或企业客户端进程。关闭开关只停止自动收发服务，不会退出微信或删除 SQLite 状态。
