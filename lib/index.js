...
function onboardingSummary(bot) {
  const labels = {
    ...
    login_timeout: "3 分钟内未完成登录，本次任务已自动取消",
...
function startWechatProgressMonitor(exec, baseUrl, token, channelId, monitors) {
  ...
  const check = async () => {
    try {
      ...
      const now = Date.now();
      const phaseChanged = lastPhase !== null && lastPhase !== bot.phase;
      const reportDue = now - lastReportAt >= 60000;
      if (bot.phase === "connected") {
        if (lastPhase !== "connected" || reportDue) {
          followup(`持续任务“把微信机器人 ${bot.name || channelId} 真正接入并验证可用”已自行检测到登录完成并进入 connected。现在直接调用 im_channel_manage(status) 检查接收、Hook 优先与 UIA/OCR 兜底健康状态；确认后调用 im_channel_manage(confirm_wechat_ready, channelId=${channelId}) 完成任务。验收发现问题时继续诊断修复。`, "微信登录已完成，正在自动验收");
          lastReportAt = now;
        }
        lastPhase = bot.phase;
        return;
      }
      if (bot.phase === "login_timeout") {
        followup(`微信机器人 ${bot.name || channelId} 在 3 分钟观察期内未完成登录，本次接入任务已经自动取消。现在向用户简短报告超时结果并结束本次任务。`, "微信登录超时，任务已取消");
      ...
      if (phaseChanged || reportDue) {
        followup(`这是 3 分钟持续任务“把微信机器人 ${bot.name || channelId} 真正接入并验证可用”的自动检查。当前状态：${onboardingSummary(bot)}。Harness 会自行检测登录结果，Agent 持续后台观察并判断下一步；可自动修复的问题继续调用工具处理。当前需要扫码、点击登录或手机确认时，向用户做一次简短说明，随后由 Harness 继续观察。`);
        lastReportAt = now;
      }
      lastPhase = bot.phase;
    } catch (error) {
      const now = Date.now();
      if (lastPhase !== "monitor_error" || now - lastReportAt >= 60000) {
        followup(`持续任务“把微信机器人 ${channelId} 真正接入并验证可用”的心跳检查遇到问题：${error?.message || error}。现在主动诊断并尝试修复，向用户汇报正在处理的步骤，然后继续本次任务。`, "微信机器人搭建遇到问题");
        lastReportAt = now;
      }
      lastPhase = "monitor_error";
    }
  };
  monitors.set(channelId, setInterval(check, 5000));
}
...
function registerChannelManagerTool(ctx, options) {
  ...
  ctx.tools.register({
    ...
    description: "本机普通 Harness 会话使用的通道管理工具。检测当前设备并搭建、启停、诊断微信或钉钉通道，也可通过已连接的微信机器人向指定联系人主动发送消息；外部 IM 通道会话负责日常消息交互。",
    parameters: {
      type: "object",
    ...
    async execute(args, exec) {
      ...
      if (action === "setup_wechat") {
        ...
        return { action, ok: true, message: "Harness 已开始构建独立微信机器人，并将在 3 分钟内自行监控登录状态。检测到登录完成会自动继续验收，超时则自动取消并报告本次任务结果。", details: JSON.stringify(value, null, 2) };
      }
